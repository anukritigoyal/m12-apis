...
countries <- GET("http://data.unhcr.org/api/stats/country_of_residence.json")
countries.content <- content(countries, "text")
country.code <- fromJSON(countries.content)
filter(country.code, country_of_residence_en == "Syrian Arab Republic") %>% select(country_of_residence)
